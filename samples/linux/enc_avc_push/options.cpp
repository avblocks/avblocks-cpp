...
void setDefaultOptions(Options& opt)
{
    ...
    fs::path output(getExeDir() + "/../../output/enc_avc_push");
...
{
    cout << "enc_avc_push --frame <width>x<height> --rate <fps> --color <COLOR> --input <file.yuv> --output <file.h264>" << endl
    << "[--colors]" << endl;
    doHelp(cout, optcfg);
}
...
std::istringstream &operator>>(std::istringstream &in, ColorDescriptor &color)
{
    std::string name;
    in >> name;
    ColorDescriptor* desc = getColorByName(name.c_str());
    if (desc)
        color = *desc;
    return in;
}
...
ErrorCodes prepareOptions(Options &opt, int argc, char* argv[])
{
    if (argc < 2)
    {
        ...
        cout << "Using defaults:\n";
        cout << "enc_avc_push --input " << opt.yuv_file;
    ...
    (("output,o"),	opt.h264_file,          string(),		("output H264 file"))
