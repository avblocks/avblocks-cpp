...
The built executable will be placed in `bin\x64\simple_video_crop.exe`.
...
2. Run the sample from the sample directory (the input/output file paths are relative to the working directory):
    ...
    ```powershell
    cd samples\windows\simple_video_crop
    ..\..\..\bin\x64\simple_video_crop.exe
    ```

3. The cropped output file `AndrewConnolly_2014_640x480.mp4` will be created in the `samples\windows\simple_video_crop` directory.
