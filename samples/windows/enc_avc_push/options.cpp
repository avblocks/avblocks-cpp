...
void setDefaultOptions(Options &opt)
{
    ...
    fs::path output(getExeDir() + L"/../../output/enc_avc_push");
...
{
    wcout << L"enc_avc_push --frame <width>x<height> --rate <fps> --color <COLOR> --input <file.yuv> --output <file.h264>" << endl
          << L"[--colors]" << endl;
    doHelp(wcout, optcfg);
}
...
void listColors()
{
    ...
    for (int i = 0; i < color_formats_len; ++i)
    {
        ...
        wcout << std::setw(20) << std::left << color.name << color.description << endl;
    }
}
...
ErrorCodes prepareOptions(Options &opt, int argc, wchar_t* argv[])
{
    if (argc < 2)
    {
        ...
        wcout << L"enc_avc_push --input " << opt.yuv_file;
    ...
    optcfg.addOptions()
        ...
        (L"output,o", opt.h264_file, wstring(), L"output H264 file")
